...
These rules apply when the AI agent creates or edits FPI scripts:

- **Editing existing scripts**: Save changes to the same folder the original script is located in, unless specified otherwise.
- **Creating new scripts**: Save new scripts inside `scriptbank\Kilo\` (create this folder if it doesn't exist).
- **Generated script header**: Every script created or modified by the AI must include this comment at the top:

```
;Generated by FPI Scripting Agent
```

Place it right after the `;Artificial Intelligence Script` line (or at the very top of the file if that line isn't present).
